...
gsl_matrix* get_cubic_coeffs(const gsl_vector* pebbles_coord) {
    ...
    gsl_matrix* cubics_coeffs = gsl_matrix_calloc(num_pebbles, 4);
...
*/
void spline_eval(const gsl_matrix* cubics_coeffs, size_t t_sample_resolution, gsl_matrix* eval_matrix) {
...
/*
    ...
    The resulting path is returned in the form of a gsl_matrix.
*/
gsl_matrix* create_path(const gsl_vector* pebbles_xy, size_t t_sample_resolution) {
    ...
    gsl_matrix* x_coeffs = get_cubic_coeffs(&pebbles_x_view.vector);
    gsl_matrix* y_coeffs = get_cubic_coeffs(&pebbles_y_view.vector);
    ...
    gsl_matrix* spline_eval_m = gsl_matrix_alloc(x_coeffs->size1 * 2, t_sample_resolution);
    gsl_matrix_view x_path = gsl_matrix_submatrix(spline_eval_m, 0, 0, x_coeffs->size1, t_sample_resolution);
    gsl_matrix_view y_path = gsl_matrix_submatrix(spline_eval_m, x_coeffs->size1, 0, x_coeffs->size1, t_sample_resolution);
